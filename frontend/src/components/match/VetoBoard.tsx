...
const calculateState = (vetoLog: VetoLog[], seriesType: string, gameMode: string = 'competitive') => {
  ...
  if (gameMode === 'wingman') {
    if (seriesType === 'BO1') {
      ...
      if (turnIndex >= 4) return { turn: 'FINISHED', type: 'NONE', mapRef: null };
      ...
      if (turnIndex >= 4) return { turn: 'FINISHED', type: 'NONE', mapRef: null };
      turnTeam = turnIndex % 2 === 0 ? 'TEAM1' : 'TEAM2';
      if (turnIndex === 0) type = 'BAN';
      else if (turnIndex < 3) type = 'PICK';
      else type = 'BAN';
    }
  ...
  // COMPETITIVE (7 maps)
  if (seriesType === 'BO1') {
    if (turnIndex >= 6) return { turn: 'FINISHED', type: 'NONE', mapRef: null };
    turnTeam = turnIndex % 2 === 0 ? 'TEAM1' : 'TEAM2';
    type = 'BAN';
  } else if (seriesType === 'BO3') {
    if (turnIndex >= 6) return { turn: 'FINISHED', type: 'NONE', mapRef: null };
...
export default function VetoBoard({ match, mySlot }: Props) {
  ...
  // 1. Load Map Pool từ DB theo game_mode
  useEffect(() => {
    const gameMode = match.game_mode || 'competitive';
    api.get(`/api/matches/maps/active?game_mode=${gameMode}`).then(res => setMaps(res.data));
  }, [match.game_mode]);

  // 2. Tính toán trạng thái hiện tại với game_mode
  const currentState = useMemo(() =>
    calculateState(match.veto_log || [], match.series_type, match.game_mode || 'competitive'),
    [match.veto_log, match.series_type, match.game_mode]);
